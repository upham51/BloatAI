      ...
      <div className="flex justify-between gap-2 mb-3">
        {[1, 2, 3, 4, 5].map((rating) => (
          <motion.button
            ...
            className="flex-1 flex flex-col items-center gap-1 py-2 px-1 rounded-xl bg-white/50 hover:bg-white/80 transition-all shadow-sm"
          >
            <span className="text-2xl">{RATING_EMOJIS[rating]}</span>
            <span className="text-2xs text-muted-foreground font-bold">{RATING_LABELS[rating]}</span>
          </motion.button>
        ))}
      </div>
      <button
